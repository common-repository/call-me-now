...
class Call_Me_Now_Widget extends WP_Widget {
	...
	public function form($instance) {
		...
		if (empty($title)) {
			$title = __('Call Me Now', 'call_me_now');
		} else {
			$title 	= ( $title && $title !=='' ? $title : __('Call Me Now', 'call_me_now') );
		}
		
		if (empty($label)) {
			$label = __('Phonenumber', 'call_me_now');
		} else {
			$label 	= ( $label && $label !=='' ? $label : __('Phonenumber', 'call_me_now') );
		}

		if (empty($button)) {
			$button = __('Call Me Now', 'call_me_now');
		} else {
			$button = ( $button && $button !=='' ? $button : __('Call Me Now', 'call_me_now') );
		}

		if (empty($class)) {
			$class = __('Call Me Now', 'call_me_now');
		} else {
			$class = ( $class && $class !=='' ? $class : __('Call Me Now', 'call_me_now') );
		}
